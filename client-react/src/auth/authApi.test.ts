...
describe("authApi", () => {
  ...
  describe("login", () => {
    it("returns tokens on success", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse<AuthTokens>({ ok: true, body: mockTokens }),
      );
    ...
    it("throws error on failure with message", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse<{ error: string }>({ ok: false, body: { error: "Invalid credentials" } }),
      );
    ...
    it("throws generic error when json parsing fails", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse({ ok: false, jsonError: new Error("Parse error") }),
      );
      await expect(login("test@example.com", "wrong")).rejects.toThrow("Login failed");
    });
  });
  ...
  describe("register", () => {
    ...
    it("throws error on failure", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse<{ error: string }>({ ok: false, body: { error: "Email already exists" } }),
      );
  ...
  describe("forgotPassword", () => {
    ...
    it("throws error on failure", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse<{ error: string }>({ ok: false, body: { error: "Email not found" } }),
      );
  ...
  describe("resetPassword", () => {
    ...
    it("throws error on failure", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse<{ error: string }>({ ok: false, body: { error: "Invalid token" } }),
      );
  ...
  describe("fetchProviders", () => {
    it("returns providers on success", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse({ ok: true, body: { google: true, apple: false, phone: true } }),
      );
  ...
  describe("sendOtp", () => {
    ...
    it("throws error on failure", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse<{ error: string }>({ ok: false, body: { error: "Invalid phone number" } }),
      );
  ...
  describe("verifyOtp", () => {
    ...
    it("throws error on failure", async () => {
      vi.mocked(apiClient.apiCall).mockResolvedValue(
        mockResponse<{ error: string }>({ ok: false, body: { error: "Invalid code" } }),
      );
