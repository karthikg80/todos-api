...
export function TodoRow({
  ...
  const rowClass = `todo-item${isActive ? " todo-item--active" : ""}${isExpanded ? " todo-item--expanded" : ""}${todo.completed ? " completed todo-item--completed" : ""}${isSelected ? " todo-item--selected" : ""}${isOverdue ? " todo-item--overdue" : ""}${isEntering ? " todo-item--entering" : ""}${isExiting ? " todo-item--exiting" : ""}${justCompleted ? " todo-item--just-completed" : ""}`;
  ...
  return (
    ...
    >
      ...
      <div className="todo-content">
        ...
        {chips.length > 0 && (
          <div className="todo-content__meta">
            <div className="todo-chips">
              {chips.map((chip) => (
                <span
                  key={chip.key}
                  className={`todo-chip todo-chip--${chip.variant} todo-chip--family-${chip.family}`}
                  onClick={
                    chip.variant === "tag"
                      ? (e) => {
                          e.stopPropagation();
                          onTagClick?.(chip.label.slice(1));
                        }
                      : undefined
                  }
                >
              ...
              (() => {
                ...
                return (
                  <div className="todo-subtask-bar">
                    ...
                    <span className="todo-subtask-bar__label">
                      {done} of {total}
                    </span>
                  </div>
                );
          ...
          <div className="todo-inline-actions">
            {!todo.completed && todo.status !== "cancelled" && (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onLifecycleAction(todo.id, "snooze-tomorrow");
                }}
              >
                <IconClock size={13} />
              </button>
            ...
            {todo.status === "cancelled" ? (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onLifecycleAction(todo.id, "reopen");
                }}
              >
                <IconRefresh size={13} />
              </button>
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onLifecycleAction(todo.id, "cancel");
                }}
              >
                <IconXCircle size={13} />
              </button>
            ...
            {todo.completed && !todo.archived && (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onLifecycleAction(todo.id, "archive");
                }}
              >
                <IconArchive size={13} />
              </button>
                ...
                {!todo.completed && todo.status !== "cancelled" && (
                  <>
                    ...
                    <button
                      ...
                      onClick={() => {
                        setMenuOpen(false);
                        onLifecycleAction(todo.id, "snooze-next-week");
                      }}
                    >
                      Snooze → Next week
                    </button>
                    <button
                      ...
                      onClick={() => {
                        setMenuOpen(false);
                        onLifecycleAction(todo.id, "snooze-next-month");
                      }}
                    >
                      Snooze → Next month
                    </button>
