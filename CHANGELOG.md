...
- PR #41 (merged): Added feature-flagged enhanced Task Critic panel scaffolding (structured sections, reusable actions) with default OFF behavior.
...
- PR #41 (merged): Added enhanced critic panel shell with score summary, improvements section, feedback input, apply/dismiss actions, and collapsed "Future insights" section.
...
- PR #42 (merged): Added client-side `.ics` export for currently filtered due-dated todos.
- PR #42 (merged): Export generates `todos-YYYY-MM-DD.ics` and includes `VCALENDAR` + `VEVENT` entries with all-day due-date events.
...
- PR #41 (merged): Added UI coverage for Task Critic feature-flag OFF/ON rendering and apply/dismiss behavior (`tests/ui/ai-critic-panel.spec.ts`).
- PR #42 (merged): Added UI coverage for ICS export disabled state, filtered inclusion, and generated calendar content (`tests/ui/ics-export.spec.ts`).
- PR #43 (merged): Added baseline changelog documentation for M1-M3 and tester validation notes.
