...
describe('PrismaTodoService (Integration)', () => {
  ...
  describe('create', () => {
    ...
    it('should persist created todo across sessions', async () => {
      const created = await service.create(TEST_USER_ID, { title: 'Persistent Todo' });
  ...
  describe('findById', () => {
    it('should find todo by ID from database', async () => {
      const created = await service.create(TEST_USER_ID, { title: 'Test Todo' });
  ...
  describe('update', () => {
    it('should update todo title in database', async () => {
      const created = await service.create(TEST_USER_ID, { title: 'Original Title' });
      const updated = await service.update(TEST_USER_ID, created.id, { title: 'Updated Title' });
    ...
    it('should update todo description', async () => {
      const created = await service.create(TEST_USER_ID, { title: 'Test', description: 'Original' });
      const updated = await service.update(TEST_USER_ID, created.id, { description: 'Updated' });
    ...
    it('should update todo completed status', async () => {
      ...
      const updated = await service.update(TEST_USER_ID, created.id, { completed: true });
    ...
    it('should return null for non-existent ID', async () => {
      const updated = await service.update(TEST_USER_ID, 'non-existent-id', { title: 'Test' });
      expect(updated).toBeNull();
    });

    it('should return null for invalid UUID', async () => {
      const updated = await service.update(TEST_USER_ID, 'invalid-uuid', { title: 'Test' });
      expect(updated).toBeNull();
    });
    ...
    it('should update the updatedAt timestamp', async () => {
      ...
      const updated = await service.update(TEST_USER_ID, created.id, { title: 'Updated' });
  ...
  describe('delete', () => {
    ...
    it('should persist deletion across sessions', async () => {
      const created = await service.create(TEST_USER_ID, { title: 'To Delete' });
  ...
  describe('database constraints', () => {
    ...
    it('should enforce description max length (1000 chars)', async () => {
      ...
      await expect(
        service.create(TEST_USER_ID, { title: 'Test', description: longDescription })
      ).rejects.toThrow();
    });
  ...
  describe('userId isolation', () => {
    ...
    it('should not allow user to access another users todo', async () => {
      const user1Todo = await service.create(TEST_USER_ID, { title: 'User 1 Todo' });
    ...
    it('should not allow user to update another users todo', async () => {
      ...
      const updated = await service.update(TEST_USER_ID_2, user1Todo.id, { title: 'Hacked' });
      expect(updated).toBeNull();
    ...
    it('should cascade delete todos when user is deleted', async () => {
      ...
      const dbTodos = await prisma.todo.findMany({ where: { userId: TEST_USER_ID } });
